...
export class Fetch {
  ...
  static async getBlob(ur: string, header?: any): Promise<Blob> {
    if (this._adapter === "axios") {
      const response = await axios.get(`${this._baseUrl}${ur}`, { ...header, responseType: "blob" })
      return response.data as Blob;
    } else {
      const response = await fetch(`${this._baseUrl}${ur}`, header)
      if (!response.ok) {
